...
adapter_range""       # Default adaptor range
...
echo "Adapter range: $adapter_range"
...
command_multi_args=""
[[ "$remove_n" == "true" ]] && command_multi_args+="-rn "
[[ "$remove_c" == "true" ]] && command_multi_args+="-rc "
command_multi_args+="-a $adapter_range"

echo "Command Args: $command_args"  # デバッグ出力

# Pythonスクリプトの実行
# custom_outputが指定されているか確認
if [ -n "$custom_output" ]; then
    # custom_outputが指定されている場合、Pythonスクリプトの実行をスキップ
    output="$custom_output"
else
    # アダプター範囲が設定されているか確認
    if [[ -z "$adapter_range" ]]; then
        # アダプター範囲が設定されていない場合、標準のPythonスクリプトを使用
        command_args="-l $linker_range"
        output=$("${CONDA_PATH}"/bin/python "${GENSH_PATH}"/input_recog.py "$input_pdb_path" $command_args)
    else
        # アダプター範囲が設定されている場合、特別なPythonスクリプトを使用
        command_multi_args="-a $adapter_range -l $linker_range"
        output=$("${CONDA_PATH}"/bin/python "${GENSH_PATH}"/input_multi_recog.py "$input_pdb_path" $command_multi_args)
    fi
fi
...
do
    ...
    if [ -f "$file" ]; then
        ...
        eval_output=$("${CONDA_PATH}"/bin/python "${GENSH_PATH}"/MPNN-prep.py "$dir_name/$(basename "$file")")
...
for fa_file in $(find "$output_mpnn_dir" -name '*.fa'); do
    ...
    /^>/ {
        if (seq) { gsub("/", ":", seq); print seq > filename }  # 前のシーケンスをファイルに書き込む前に置換
        seq = ""  # シーケンスをリセット
        if (/sample=/) {
    ...
    END {
        if (seq) { gsub("/", ":", seq); print seq > filename }  # 最後のシーケンスをファイルに書き込む前に置換
    }' "$fa_file"
done
